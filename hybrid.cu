...
void boruvka()
{
	...
	cudppDestroyPlan(scanPlan);
	 ...
	 error = cudaGetLastError();
        if(error != cudaSuccess)
        {
                printf("At line 662 CUDA error: %s\n", cudaGetErrorString(error));
        }
	...
	error = cudaGetLastError();
        if(error != cudaSuccess)
        {
                printf("At line 691  CUDA error: %s\n", cudaGetErrorString(error));
        }
	...
	error = cudaGetLastError();
        if(error != cudaSuccess)
        {
                printf("At line 726 CUDA error: %s\n", cudaGetErrorString(error));
        }	
	...
	error = cudaGetLastError();
        if(error != cudaSuccess)
        {
                printf("At line 779  CUDA error: %s\n", cudaGetErrorString(error));
        }
	...
	error = cudaGetLastError();
        if(error != cudaSuccess)
        {
                printf("At line 825  CUDA error: %s\n", cudaGetErrorString(error));
        }
